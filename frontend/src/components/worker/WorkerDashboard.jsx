...
const WorkerDashboard = () => {
  ...
  const handleAccept = async (id) => {
    try {
      await axios.put(`/worker/bookings/${id}/accept`);
      fetchData();
    } catch (error) {
  ...
  const handleReject = async (id) => {
    try {
      await axios.put(`/worker/bookings/${id}/reject`);
      fetchData();
    } catch (error) {
  ...
  const handleComplete = async (id) => {
    try {
      await axios.put(`/worker/bookings/${id}/complete`);
      fetchData();
    } catch (error) {
            ...
            <div className="schedule-table">
              ...
              <div className="table-body">
                {activeBookings.slice(0, 3).map((booking, index) => {
                  const statuses = ['In Progress', 'Up Next', 'Scheduled'];
                  ...
                  return (
                    <div key={booking._id} className="table-row">
                      ...
                      <div className="table-col time-info">
                        🕐 <span>{booking.scheduledTime || '09:30 AM - 11:00 AM'}</span>
                      </div>
