  ...
  const fetchData = async () => {
    try {
      ...
      const [analyticsRes, usersRes, workersRes, bookingsRes] = await Promise.all([
        ...
        axios.get('/admin/workers', config),
        ...
        <div className="p-8">
          ...
          <div className="grid md:grid-cols-4 gap-6 mb-8">
            ...
            <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200 hover:shadow-md transition-shadow">
              ...
              <p className="text-3xl font-bold text-gray-800 mb-1">{workers?.filter(w => w.isActive)?.length || analytics?.totalWorkers || '1,240'}</p>
            ...
            <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200 hover:shadow-md transition-shadow">
              ...
              <p className="text-3xl font-bold text-gray-800 mb-1">{analytics?.pendingBookings || bookings?.filter(b => b.status === 'pending')?.length || '84'}</p>
            ...
            <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200 hover:shadow-md transition-shadow">
              ...
              <p className="text-3xl font-bold text-gray-800 mb-1">{users?.length || analytics?.totalUsers || '3,450'}</p>
          ...
          <div className="grid md:grid-cols-2 gap-6 mb-8">
            <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200">
              <div className="flex justify-between items-center mb-4">
                ...
                <Link to="/admin/services/add" className="bg-indigo-600 text-white px-4 py-2 rounded-lg hover:bg-indigo-700 transition-colors text-sm font-medium">
                  Add Service
                </Link>
              </div>
              <Link to="/admin/services" className="text-indigo-600 hover:underline font-medium">Manage All Services →</Link>
            </div>

            <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200">
              <h2 className="text-xl font-bold text-gray-800 mb-4">Quick Stats</h2>
              <div className="space-y-2">
                <p className="text-gray-700">Pending Bookings: <span className="font-bold text-gray-900">{analytics?.pendingBookings || bookings?.filter(b => b.status === 'pending')?.length || 0}</span></p>
                <p className="text-gray-700">Completed Bookings: <span className="font-bold text-gray-900">{analytics?.completedBookings || bookings?.filter(b => b.status === 'completed')?.length || 0}</span></p>
                <p className="text-gray-700">Total Services: <span className="font-bold text-gray-900">{analytics?.totalServices || 0}</span></p>
              </div>
            </div>
          </div>
                        ...
                        <div className="flex items-start justify-between">
                          <div className="flex items-start space-x-4 flex-1">
                            <div className="w-12 h-12 bg-gradient-to-br from-blue-400 to-indigo-600 rounded-full flex items-center justify-center flex-shrink-0">
                              <span className="text-white font-bold text-lg">{worker.name?.charAt(0) || 'W'}</span>
                            </div>
                            <div className="flex-1 min-w-0">
                              <div className="flex items-center space-x-2 mb-1">
